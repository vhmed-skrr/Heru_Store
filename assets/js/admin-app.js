               ...
               <div style="background:var(--bg-elevated); border:1px solid var(--border); border-radius:var(--radius-md); overflow:hidden;">
                   ...
                   <div style="padding:16px;">
                       ...
                       <div style="display:flex; justify-content:space-between;">
                           <button class="action-btn" style="margin:0;" onclick="handleEditProduct('${p.id}')">تعديل المنتج</button>
...
async function loadCategories() {
    ...
    let s = '';
    (data||[]).forEach(c => { s+=`<tr><td>${c.name_ar}</td><td>${c.color||'-'}</td><td><button class="action-btn" onclick="handleEditCategory('${c.id}')">تعديل</button> <button class="action-btn" style="color:var(--error);" onclick="appAdmin.deleteCat('${c.id}')">حذف</button></td></tr>`; });
